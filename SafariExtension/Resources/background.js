...
const SAFARI_COOKIE_SNAPSHOTS_KEY = 'safariCookieSnapshots';
...
const STATE_SYNC_CONTENT_SCRIPT = 'content.js';
...
// The Safari 16.0–16.3 compatibility extension declares these permissions in
// its manifest. It deliberately has no runtime permission or dynamic-script
// APIs, so treat its declared permissions as available.
const usesLegacyStaticPermissions = DETECTED_BROWSER === 'safari' &&
...
async function refreshPermissionStateCache() {
  const [stateSync, tabSharing] = await Promise.all([
    hasOptionalPermissions(STATE_SYNC_PERMISSIONS),
    hasOptionalPermissions(TAB_SHARING_PERMISSIONS)
  ]);
  await chrome.storage.local.set({ optionalPermissionState: { stateSync, tabSharing } }).catch(() => {});
}
...
// Safari can expose a working cookies API after website access is granted while
// permissions.contains() for the combined cookies/tabs/scripting request still
// reports false. Cookie synchronization only needs the cookie capability; the
// presence of chrome.cookies is the runtime authority check.
async function canSyncCookies() {
  return await isStateSyncConfigured() && !!chrome.cookies;
}

function safariCookieSignature(cookie) {
  return JSON.stringify([
    cookie.value, cookie.domain, cookie.path || '/', cookie.expirationDate ?? null,
    cookie.secure, cookie.httpOnly, cookie.hostOnly, cookie.sameSite || null
  ]);
}

function safariCookiePayload(cookie, updatedAt = 0, removed = false) {
  return {
    name: cookie.name,
    value: cookie.value,
...
async function safariCookieSnapshot(site) {
  if (!chrome.cookies || !site || site === '*') return null;
  const cookies = await chrome.cookies.getAll({ domain: site }).catch(() => []);
  const snapshot = {};
  for (const cookie of cookies) {
    snapshot[cookieIdentity(cookie)] = {
      signature: safariCookieSignature(cookie),
      cookie: safariCookiePayload(cookie)
    };
  }
  return snapshot;
}

async function rememberSafariCookieSnapshot(site) {
  if (DETECTED_BROWSER !== 'safari') return;
  const snapshot = await safariCookieSnapshot(site);
  if (!snapshot) return;
  const stored = await chrome.storage.local.get(SAFARI_COOKIE_SNAPSHOTS_KEY).catch(() => ({}));
  const snapshots = stored[SAFARI_COOKIE_SNAPSHOTS_KEY] || {};
  snapshots[site] = snapshot;
  await chrome.storage.local.set({ [SAFARI_COOKIE_SNAPSHOTS_KEY]: snapshots }).catch(() => {});
}

// Safari does not implement cookies.onChanged. Compare against a persisted
// per-site baseline when an observed page finishes loading or pings its
// background port, and send only actual cookie additions, edits, and removals.
async function syncSafariCookieChangesForTab(tab) {
  if (DETECTED_BROWSER !== 'safari' || !tab?.url || !(await canSyncCookies())) return;
  const site = syncDomainForHostname(safeHostname(tab.url));
  if (site === '*') return;

  const { appSettings = {} } = await chrome.storage.local.get('appSettings').catch(() => ({}));
  if (appSettings.automaticSync !== true ||
      appSettings.stateParticipatingBrowsers?.[DETECTED_BROWSER] !== true ||
      !isSiteStateSyncEnabled(site, appSettings)) return;

  const current = await safariCookieSnapshot(site);
  if (!current) return;
  const stored = await chrome.storage.local.get(SAFARI_COOKIE_SNAPSHOTS_KEY).catch(() => ({}));
  const snapshots = stored[SAFARI_COOKIE_SNAPSHOTS_KEY] || {};
  const previous = snapshots[site];
  snapshots[site] = current;
  await chrome.storage.local.set({ [SAFARI_COOKIE_SNAPSHOTS_KEY]: snapshots }).catch(() => {});

  // The first observation establishes the baseline. A later completed login
  // navigation or page heartbeat is what supplies the changed cookie set.
  if (!previous) return;

  const updatedAt = Date.now();
  const changes = [];
  for (const [key, entry] of Object.entries(current)) {
    if (previous[key]?.signature === entry.signature) continue;
    const changed = { ...entry.cookie, updatedAt };
    await setCookieTimestamp(changed, updatedAt);
    changes.push(changed);
  }
  for (const [key, entry] of Object.entries(previous)) {
    if (current[key]) continue;
    const removed = { ...entry.cookie, removed: true, updatedAt };
    await setCookieTimestamp(removed, updatedAt);
    await chrome.storage.local.set({ [`tombstone_cookies_${key}`]: removed }).catch(() => {});
    changes.push(removed);
  }
  if (changes.length === 0) return;

  console.log(`[BrowSync] Safari cookie fallback detected ${changes.length} change(s) for ${site}`);
  send({
    type: 'sync', browser: DETECTED_BROWSER, site, category: 'cookies',
    payload: { kind: 'cookies', cookies: changes },
    messageId: crypto.randomUUID(), timestamp: updatedAt
  });
}
...
async function reconcileOptionalFeaturePermissions() {
  if (usesLegacyStaticPermissions) return;
  if (!permissionsAPI?.remove || await isStateSyncConfigured()) return;
  // The popup sends its settings write only after requestFeaturePermissions()
  ...
  // State Sync is the only feature that needs cookies, scripting, and website
  // access. Keep tabs only while Tab Sharing still uses it.
  const permissions = ['cookies', 'scripting'];
  if (!(await isTabSharingConfigured())) permissions.push('tabs');
  try {
    await permissionsAPI.remove({ permissions, origins: ['*://*/*'] });
  } catch (error) {
...
async function reconcileStateContentScript() {
  if (!chrome.scripting?.getRegisteredContentScripts) {
    // The legacy Safari target uses a manifest-declared content script. It is
    // intentionally static, while its behavior remains gated by App settings.
    await chrome.storage.local.set({
      stateSyncRuntimeEnabled: usesLegacyStaticPermissions && await isStateSyncConfigured()
    }).catch(() => {});
  ...
  if (port.name === 'browsync-keepalive') {
    port.onMessage.addListener((msg) => {
      if (msg.type === 'ping') {
        ...
        void syncSafariCookieChangesForTab(port.sender?.tab || { url: msg.url });
      }
    });
  }
});
...
async function sendCookiesSnapshot(site) {
  if (!(await canSyncCookies())) return;
...
async function requestSiteStateSync(site) {
  ...
  send({
    type: 'pull', browser: DETECTED_BROWSER, category: 'browserData', site,
    messageId: crypto.randomUUID(), timestamp: Date.now()
  });
...
async function handleBookmarkChange(reason, event = {}) {
  ...
  bookmarkDebounceTimer = setTimeout(async () => {
    ...
    // Safari's event is delivered after the bookmark tree has changed. Keep
    // the app's existing snapshot as the pre-change baseline so a reorder is
    // compared against the prior order and can produce a notification.
...
// "cookies" is optional, so this listener must also be attached after the
// user grants it instead of only once during extension startup. Safari does
// not support cookies.onChanged, so its polling fallback above owns outgoing
// cookie detection when this event is absent.
let cookiesListenerAttached = false;
function attachCookiesListener() {
  if (cookiesListenerAttached || !chrome.cookies?.onChanged?.addListener) return;
...
async function applyCookieSync(cookies) {
  ...
  for (const cookie of cookies) {
    ...
    if (localCookies.has(cookieKey) && (localTimestamps[cookieKey] || 0) >= updatedAt) continue;
  ...
  // Keep Safari's polling baseline aligned with incoming changes so its next
  // heartbeat cannot echo a remote cookie back as a new local update.
  if (DETECTED_BROWSER === 'safari') {
    const sites = new Set(cookies.map(cookie =>
      syncDomainForHostname((cookie.domain || '').replace(/^\./, '').toLowerCase())
    ));
    await Promise.all([...sites].map(rememberSafariCookieSnapshot));
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'UPDATE_SETTING') {
    ...
    // Write first and wait for reconciliation before the popup asks Safari for
    // a permission. Otherwise permissions.onAdded can see stale settings and
  ...
  (async () => {
    ...
    if (message.type === 'heartbeat_ping') {
      await syncSafariCookieChangesForTab(sender.tab || { url: message.url });
      return;
    }
...
async function showStateSyncUpdatePrompt(cookies) {
  ...
  // aid, not a list of currently open pages. Otherwise a remote login reaches
  // Safari and is applied, but an already-open matching tab never gets the
  // reload banner.
...
// "tabs" is optional for both State Sync and Tab Sharing. Attach after a
// runtime grant as well as at startup.
...
function attachTabsListener() {
  ...
  chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    if (changeInfo.status === 'loading' && tab.url) {
      pendingSiteReloads.delete(tabId);
      // Safari has no cookies.onChanged. Capture the pre-navigation baseline
      // so a login redirect can be detected as soon as the page completes.
      if (DETECTED_BROWSER === 'safari') void rememberSafariCookieSnapshot(syncDomainForHostname(safeHostname(tab.url)));
    ...
    if (DETECTED_BROWSER === 'safari') void syncSafariCookieChangesForTab(tab);
    if (!(await canShareTabs())) return;
...
// Users can add or remove optional permissions from Safari's Extensions
// settings while the popup is closed. Keep listeners and registration aligned.
if (permissionsAPI?.onAdded) {
  permissionsAPI.onAdded.addListener(() => {
    attachCookiesListener();
    attachTabsListener();
    void refreshPermissionStateCache();
    void reconcileStateContentScript();
  });
}
if (permissionsAPI?.onRemoved) {
  permissionsAPI.onRemoved.addListener(() => {
    void refreshPermissionStateCache();
    void reconcileStateContentScript();
  });
}
